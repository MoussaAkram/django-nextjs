...
export default function Register() {
  const [formData, setFormData] = useState<User>({ username: '', email: '', password: '', confirmation: '' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (formData.username == '' || formData.email == '' || formData.password == '' || formData.confirmation == '' ) {
      setErrorMessage('Please fill in all fields.');
    } else {
      setErrorMessage('');
      postLog(formData);
      setFormData({ username: '', email: '', password: '', confirmation: '' });
    }
  ...
  async function postLog(addFormData: User) {
    await URLS.Register(addFormData).then(response => {
      router.push('/movies')
    })
      .catch(e => {
                  ...
                  <div className="flex items-center justify-center ">
                    <div className="bg-black flex flex-col w-80 border border-gray-900 rounded-lg px-8 py-10">
                      ...
                      <form onSubmit={handleSubmit} className="flex flex-col space-y-5 mt-5">
                        ...
                        <label htmlFor='email_input' className="font-bold text-lg text-white " >Email:</label>
                        <input id="email_input" value={formData.email || ''} onChange={handleChange} name='email' type='email' placeholder="email" className="border rounded-lg py-3 px-3 mt-2 bg-black border-indigo-600 placeholder-white-500 text-white" />
                        ...
                        <label htmlFor='confirmation' className="font-bold text-lg text-white">Confirmation:</label>
                        <input id="confirmation" value={formData.confirmation || ''} onChange={handleChange} name='confirmation' type="password" placeholder="****" className="border rounded-lg py-3 px-3 bg-black border-indigo-600 placeholder-white-500 text-white" />
                        <div className='pt-4'>
                          <button className="border h-full w-full border-indigo-600 bg-black text-white rounded-lg py-3 font-semibold" >Sign up</button>
                      ...
                      <p className="text-sm font-light text-gray-400 mt-5">
                        Already have an account? <Link href="/login" className="font-medium text-primary-600 hover:underline dark:text-primary-500">Log In here.</Link>
                      </p>
                    </div>
                  </div>
